...
def main(argv):
    try:
        ...
        if cmd == "call" or cmd == "sendtx":
            ...
            fn_name = params["func"]
            fn_value = inputparams[3]
            fn_args = inputparams[4:]
            print("INFO >> {} {} , address: {}, func: {}, value:{}, args:{}"
                  .format(cmd, contractname, address, fn_name, fn_value, fn_args))
            if cmd == "call":
                result = tx_client.call_and_decode(fn_name, fn_args)
                print("INFO >> {} result: {}".format(cmd, result))
            if cmd == "sendtx":
                receipt = tx_client.send_transaction_getReceipt_v2(fn_name, fn_args, fn_value)[0]
