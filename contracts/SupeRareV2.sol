...
contract SupeRareV2 is ERC721Token, Ownable, ERC721Metadata, ParallelContract {
    using SafeMath for uint256;
    ...
    */
    function bid(uint256 _tokenId) public payable {
        ...
        returnCurrentBid(currentBid, currentBidder, _tokenId);
    ...
    function cancelBid(uint256 _tokenId) public {
        ...
        msg.sender.call.value(bidAmount)();
        clearBid(_tokenId);
        address(0x1008).call.value(0)();
    ...
    * @param _tokenId uint256 ID of the token with the standing bid
    */
    function returnCurrentBid(uint256 currentBid, address currentBidder, uint256 _tokenId) private {
    ...
    */
    function payout(uint256 _val, address _maintainer, address _creator, address _tokenOwner, uint256 _tokenId) private {
        ...
        _maintainer.call.value(maintainerPayment)();
        _creator.call.value(creatorPayment)();
        _tokenOwner.call.value(ownerPayment)();
    ...
    function enableParallel() public
    {
        ...
        // registerParallelFunctions("fillOrder(bytes32,address,address,address[5],uint256[6],uint256,bool,uint8,bytes32,bytes32)",3,"cancelOrder(bytes32,address,address,address[5],uint256[6],uint256)",3); // critical: string string
        registerParallelFunctions("bid(uint256)",1,"acceptBid(uint256)",1); // critical: string string
        registerParallelFunctions("acceptBid(uint256)",1,"cancelBid(uint256)",1); 
        registerParallelFunctions("buy(uint256)",1,"setSalePrice(uint256,uint256)",1); 
    } 
    function disableParallel() public
    {
    }
